...
type _GetDataUsageResponse GetDataUsageResponse
...
func (o *GetDataUsageResponse) UnmarshalJSON(data []byte) (err error) {
	...
	rawEntries, rawEntriesPresent := cxsdkRawFields["entries"]
	if rawEntriesPresent {
		delete(cxsdkRawFields, "entries")
	}
	...
	varGetDataUsageResponse := _GetDataUsageResponse{}

	decoder := json.NewDecoder(bytes.NewReader(strippedData))
	err = decoder.Decode(&varGetDataUsageResponse)

	if err != nil {
		return err
	}

	*o = GetDataUsageResponse(varGetDataUsageResponse)

	if rawEntriesPresent {
		var rawEntriesElements []json.RawMessage
		if jerr := json.Unmarshal(rawEntries, &rawEntriesElements); jerr == nil {
			decodedEntries := make([]DataUsageEntry, 0, len(rawEntriesElements))
			for _, rawEntriesElement := range rawEntriesElements {
				var elem DataUsageEntry
				if jerr := json.Unmarshal(rawEntriesElement, &elem); jerr != nil {
					continue
				}
				decodedEntries = append(decodedEntries, elem)
			}
			o.Entries = decodedEntries
		}
	}
