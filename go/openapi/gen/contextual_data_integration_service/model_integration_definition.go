...
type _IntegrationDefinition IntegrationDefinition
...
func (o *IntegrationDefinition) UnmarshalJSON(data []byte) (err error) {
	...
	varIntegrationDefinition := _IntegrationDefinition{}

	decoder := json.NewDecoder(bytes.NewReader(strippedData))
	err = decoder.Decode(&varIntegrationDefinition)

	if err != nil {
		return err
	}

	*o = IntegrationDefinition(varIntegrationDefinition)

	if rawRevisionsPresent {
		var rawRevisionsElements []json.RawMessage
		if jerr := json.Unmarshal(rawRevisions, &rawRevisionsElements); jerr == nil {
			decodedRevisions := make([]IntegrationRevision, 0, len(rawRevisionsElements))
			for _, rawRevisionsElement := range rawRevisionsElements {
				var elem IntegrationRevision
