...
func (c *internalDataprimeQueryServiceClient) Query(ctx context.Context, in *QueryRequest, opts ...grpc.CallOption) (InternalDataprimeQueryService_QueryClient, error) {
	stream, err := c.cc.NewStream(ctx, &InternalDataprimeQueryService_ServiceDesc.Streams[0], InternalDataprimeQueryService_Query_FullMethodName, opts...)
	if err != nil {
		return nil, err
	}
	x := &internalDataprimeQueryServiceQueryClient{stream}
...
func _InternalDataprimeQueryService_Query_Handler(srv interface{}, stream grpc.ServerStream) error {
	...
	return srv.(InternalDataprimeQueryServiceServer).Query(m, &internalDataprimeQueryServiceQueryServer{stream})
}
