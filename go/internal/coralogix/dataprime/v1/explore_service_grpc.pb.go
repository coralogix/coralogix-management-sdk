...
func (c *dataprimeExploreServiceClient) RunDataprimeQuery(ctx context.Context, in *RunDataprimeQueryRequest, opts ...grpc.CallOption) (DataprimeExploreService_RunDataprimeQueryClient, error) {
	stream, err := c.cc.NewStream(ctx, &DataprimeExploreService_ServiceDesc.Streams[0], DataprimeExploreService_RunDataprimeQuery_FullMethodName, opts...)
	if err != nil {
		return nil, err
	}
	x := &dataprimeExploreServiceRunDataprimeQueryClient{stream}
...
func _DataprimeExploreService_RunDataprimeQuery_Handler(srv interface{}, stream grpc.ServerStream) error {
	...
	return srv.(DataprimeExploreServiceServer).RunDataprimeQuery(m, &dataprimeExploreServiceRunDataprimeQueryServer{stream})
}
