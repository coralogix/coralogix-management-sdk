...
func (c *schemaStoreQueryServiceClient) Query(ctx context.Context, in *QueryRequest, opts ...grpc.CallOption) (SchemaStoreQueryService_QueryClient, error) {
	stream, err := c.cc.NewStream(ctx, &SchemaStoreQueryService_ServiceDesc.Streams[0], SchemaStoreQueryService_Query_FullMethodName, opts...)
	if err != nil {
		return nil, err
	}
	x := &schemaStoreQueryServiceQueryClient{stream}
...
func _SchemaStoreQueryService_Query_Handler(srv interface{}, stream grpc.ServerStream) error {
	...
	return srv.(SchemaStoreQueryServiceServer).Query(m, &schemaStoreQueryServiceQueryServer{stream})
}
